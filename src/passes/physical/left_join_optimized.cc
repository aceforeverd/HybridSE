        ...
        case vm::kPhysicalOpProject: {
            ...
            if (node::ExprListNullOrEmpty(window_agg_op->window_.partition_.keys_) &&
                (nullptr == window_agg_op->window_.sort_.orders_ ||
                 node::ExprListNullOrEmpty(window_agg_op->window_.sort_.orders_->order_expressions_))) {
            ...
            if (!CheckExprDependOnChildOnly(window_agg_op->window_.sort_.orders_->order_expressions_, left_schemas_ctx)
                     .isOK()) {
