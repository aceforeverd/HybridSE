...
uint32_t CalcTotalLength(uint32_t primary_size, uint32_t str_field_cnt, uint32_t str_size, uint32_t* str_addr_space) {
...
int32_t GetStrField(const int8_t* row, uint32_t idx, uint32_t str_field_offset, uint32_t next_str_field_offset,
                    uint32_t str_start_offset, uint32_t addr_space, const char** data, uint32_t* size,
                    int8_t* is_null) {
    if (row == nullptr || IsNullAt(row, idx)) {
        ...
        *is_null = false;
        return GetStrFieldUnsafe(row, idx, str_field_offset, next_str_field_offset, str_start_offset, addr_space, data,
                                 size);
    }
}

int32_t GetStrFieldUnsafe(const int8_t* row, uint32_t col_idx, uint32_t field_offset, uint32_t next_str_field_offset,
                          uint32_t str_start_offset, uint32_t addr_space, const char** data, uint32_t* size) {
    ...
    switch (addr_space) {
        case 1: {
            ...
            if (next_str_field_offset > 0) {
                next_str_offset = (uint8_t)(*(row_with_offset + next_str_field_offset));
            }
            break;
        }
        case 2: {
        ...
        *size = total_length - str_offset;
        ...
        if (next_str_offset < str_offset) {
            LOG(WARNING) << "fail to get str field, next_str_offset < "
                         ...
                         << next_str_offset << ", str_offset=" << str_offset << ", field_offset=" << field_offset
                         << ", next_str_field_offset=" << next_str_field_offset << ", addr_space=" << addr_space
                         << ", buf=" << (uint64_t)row;
            *size = 0;
            return -3;
        }
...
int32_t AppendString(int8_t* buf_ptr, uint32_t buf_size, uint32_t col_idx, int8_t* val, uint32_t size, int8_t is_null,
                     uint32_t str_start_offset, uint32_t str_field_offset, uint32_t str_addr_space,
    ...
    if (is_null) {
        ...
        FillNullStringOffset(buf_ptr, str_start_offset, str_addr_length, str_field_offset, str_body_offset);
    ...
    switch (str_addr_space) {
        ...
        case 3: {
            ...
            *(reinterpret_cast<uint8_t*>(ptr_offset + 1)) = (str_body_offset & 0xFF00) >> 8;
            *(reinterpret_cast<uint8_t*>(ptr_offset + 2)) = str_body_offset & 0x00FF;
            break;
        }
...
int32_t GetStrCol(int8_t* input, int32_t row_idx, uint32_t col_idx, int32_t str_field_offset,
    ...
    switch (type) {
        case hybridse::type::kVarchar: {
            new (data) StringColumnImpl(w, row_idx, col_idx, str_field_offset, next_str_field_offset, str_start_offset);
            break;
        }
        default: {
            return -2;
...
int32_t GetCol(int8_t* input, int32_t row_idx, uint32_t col_idx, int32_t offset, int32_t type_id, int8_t* data) {
    hybridse::type::Type type = static_cast<hybridse::type::Type>(type_id);
    if (nullptr == input || nullptr == data) {
        return -2;
    }
    ...
    switch (type) {
        ...
        default: {
            LOG(WARNING) << "cannot get col for type " << ::hybridse::type::Type_Name(type) << " type id " << type_id;
            return -2;
        }
    }
    return 0;
...
int32_t GetInnerRangeList(int8_t* input, int64_t start_key, int64_t start_offset, int64_t end_offset, int8_t* data) {
    if (nullptr == input || nullptr == data) {
        return -2;
    }
    ListV<Row>* w = reinterpret_cast<ListV<Row>*>(input);
    uint64_t start = start_key + start_offset < 0 ? 0 : start_key + start_offset;
    uint64_t end = start_key + end_offset < 0 ? 0 : start_key + end_offset;
    new (data) InnerRangeList<Row>(w, start, end);
    return 0;
}

int32_t GetInnerRowsList(int8_t* input, int64_t start_rows, int64_t end_rows, int8_t* data) {
    if (nullptr == input || nullptr == data) {
        return -2;
    }
    ListV<Row>* w = reinterpret_cast<ListV<Row>*>(input);
