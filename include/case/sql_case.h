...
class SqlCase {
 public:
    ...
    const std::string& cluster_request_plan() const { return cluster_request_plan_; }
    ...
    const bool standard_sql_compatible() const { return standard_sql_compatible_; }
    ...
    bool BuildCreateSqlFromInput(int32_t input_idx, std::string* sql, int partition_num = 1) const;
    bool BuildInsertSqlFromInput(int32_t input_idx, std::string* sql) const;
    bool BuildInsertSqlListFromInput(int32_t input_idx, std::vector<std::string>* sql_list) const;
    ...
    bool ExtractOutputData(std::vector<hybridse::codec::Row>& rows) const;  // NOLINT

    bool AddInput(const TableInfo& table_data);
    static bool TypeParse(const std::string& row_str, hybridse::type::Type* type);
    static bool TTLTypeParse(const std::string& type_str, ::hybridse::type::TTLType* type);
    ...
    static bool ExtractSchema(const std::string& schema_str,
                              type::TableDef& table);  // NOLINT
    static bool BuildCreateSqlFromSchema(const type::TableDef& table, std::string* create_sql,
    ...
    static bool ExtractIndex(const std::vector<std::string>& indexs,
                             type::TableDef& table);  // NOLINT
    static bool ExtractTableDef(const std::vector<std::string>& columns, const std::vector<std::string>& indexs,
                                type::TableDef& table);  // NOLINT
    static bool ExtractTableDef(const std::string& schema_str, const std::string& index_str,
                                type::TableDef& table);  // NOLINT
    static bool ExtractRows(const vm::Schema& schema, const std::string& data_str,
                            std::vector<hybridse::codec::Row>& rows);  // NOLINT
    static bool ExtractRows(const vm::Schema& schema, const std::vector<std::vector<std::string>>& row_vec,
                            std::vector<hybridse::codec::Row>& rows);  // NOLINT
    static bool BuildInsertSqlFromData(const type::TableDef& table, std::string data, std::string* insert_sql);
    static bool BuildInsertValueStringFromRow(const type::TableDef& table, const std::vector<std::string>& item_vec,
                                              std::string* create_sql);
    static bool BuildInsertSqlFromRows(const type::TableDef& table, const std::vector<std::vector<std::string>>& rows,
                                       std::string* insert_sql);
    static bool ExtractRow(const vm::Schema& schema, const std::string& row_str, int8_t** out_ptr, int32_t* out_size);
    static bool ExtractRow(const vm::Schema& schema, const std::vector<std::string>& item_vec, int8_t** out_ptr,
                           int32_t* out_size);
    static bool CreateTableInfoFromYamlNode(const YAML::Node& node, SqlCase::TableInfo* output);
    static bool CreateExpectFromYamlNode(const YAML::Node& schema_data, const YAML::Node& expect_provider,
                                         SqlCase::ExpectInfo* table);
    static bool LoadSchemaAndRowsFromYaml(const std::string& cases_dir, const std::string& resource_path,
                                          type::TableDef& table,                     // NOLINT
                                          std::vector<hybridse::codec::Row>& rows);  // NOLINT
    static bool CreateSqlCasesFromYaml(const std::string& cases_dir, const std::string& yaml_path,
                                       std::vector<SqlCase>& sql_case_ptr,  // NOLINT
                                       const std::string filter_mode = "");
    static bool CreateSqlCasesFromYaml(const std::string& cases_dir, const std::string& yaml_path,
                                       std::vector<SqlCase>& sql_case_ptr,  // NOLINT
                                       const std::vector<std::string>& filter_modes);
    static bool CreateTableInfoFromYaml(const std::string& cases_dir, const std::string& yaml_path,
                                        TableInfo* table_info);
    static bool CreateStringListFromYamlNode(const YAML::Node& node,
                                             std::vector<std::string>& rows);  // NOLINT
    static bool CreateRowsFromYamlNode(const YAML::Node& node,
    ...
    bool BuildCreateSpSqlFromInput(int32_t input_idx, const std::string& select_sql, const std::set<size_t>& common_idx,
                                   std::string* create_sp_sql);
    bool BuildCreateSpSqlFromSchema(const type::TableDef& table, const std::string& select_sql,
    ...
    static hybridse::sqlcase::SqlCase LoadSqlCaseWithID(const std::string& dir_path, const std::string& yaml_path,
    ...
    void SqlCaseRepeatConfig(const std::string& tag, const int value) {
        for (size_t idx = 0; idx < inputs_.size(); idx++) {
            if (inputs_[idx].repeat_tag_ == tag) {
                LOG(INFO) << "config input " << idx << " " << tag << " " << value;
