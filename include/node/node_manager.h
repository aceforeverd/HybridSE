...
class NodeManager {
 public:
    ...
    ProjectNode *MakeRowProjectNode(const int32_t pos, const std::string &name, node::ExprNode *expression);
    ProjectNode *MakeAggProjectNode(const int32_t pos, const std::string &name, node::ExprNode *expression,
                                    node::FrameNode *frame);
    PlanNode *MakeTablePlanNode(const std::string &node);
    PlanNode *MakeJoinNode(PlanNode *left, PlanNode *right, JoinType join_type, const OrderByNode *order_by,
                           const ExprNode *condition);
    // Make SQLxxx Node
    QueryNode *MakeSelectQueryNode(bool is_distinct, SqlNodeList *select_list_ptr, SqlNodeList *tableref_list_ptr,
    ...
    TableRefNode *MakeTableNode(const std::string &name, const std::string &alias);
    TableRefNode *MakeJoinNode(const TableRefNode *left, const TableRefNode *right, const JoinType type,
                               const ExprNode *condition, const std::string alias);
    TableRefNode *MakeLastJoinNode(const TableRefNode *left, const TableRefNode *right, const ExprNode *order,
                                   const ExprNode *condition, const std::string alias);
    TableRefNode *MakeQueryRefNode(const QueryNode *sub_query, const std::string &alias);
    ...
    ExprNode *MakeSimpleCaseWhenNode(ExprNode *case_expr, ExprListNode *when_list_expr, ExprNode *else_expr);
    ExprNode *MakeSearchedCaseWhenNode(ExprListNode *when_list_expr, ExprNode *else_expr);
    CallExprNode *MakeFuncNode(const std::string &name, ExprListNode *args, const SqlNode *over);
    CallExprNode *MakeFuncNode(FnDefNode *fn, ExprListNode *args, const SqlNode *over);
    CallExprNode *MakeFuncNode(const std::string &name, const std::vector<ExprNode *> &args, const SqlNode *over);
    CallExprNode *MakeFuncNode(FnDefNode *fn, const std::vector<ExprNode *> &args, const SqlNode *over);

    QueryExpr *MakeQueryExprNode(const QueryNode *query);
    SqlNode *MakeWindowDefNode(const std::string &name);
    SqlNode *MakeWindowDefNode(ExprListNode *partitions, ExprNode *orders, SqlNode *frame);
    SqlNode *MakeWindowDefNode(ExprListNode *partitions, ExprNode *orders, SqlNode *frame,
    ...
    SqlNode *MakeWindowDefNode(SqlNodeList *union_tables, ExprListNode *partitions, ExprNode *orders, SqlNode *frame,
                               bool opt_open_interval_window, bool instance_not_in_window);
    ...
    SqlNode *MakeFrameNode(FrameType frame_type, SqlNode *node_ptr, ExprNode *frame_size);
    SqlNode *MakeFrameNode(FrameType frame_type, SqlNode *node_ptr);
    SqlNode *MakeFrameNode(FrameType frame_type, SqlNode *node_ptr, int64_t maxsize);
    SqlNode *MakeFrameNode(FrameType frame_type, FrameExtent *frame_range, FrameExtent *frame_rows, int64_t maxsize);
    FrameNode *MergeFrameNode(const FrameNode *frame1, const FrameNode *frame2);
    SqlNode *MakeLimitNode(int count);

    SqlNode *MakeInsertTableNode(const std::string &table_name, const ExprListNode *column_names,
                                 const ExprListNode *values);
    SqlNode *MakeCreateTableNode(bool op_if_not_exist, const std::string &table_name, SqlNodeList *column_desc_list,
    ...
    TypeNode *MakeTypeNode(hybridse::node::DataType base, const hybridse::node::TypeNode *v1);
    TypeNode *MakeTypeNode(hybridse::node::DataType base, hybridse::node::DataType v1);
    TypeNode *MakeTypeNode(hybridse::node::DataType base, hybridse::node::DataType v1, hybridse::node::DataType v2);
    ...
    ColumnRefNode *MakeColumnRefNode(const std::string &column_name, const std::string &relation_name,
                                     const std::string &db_name);
    ColumnRefNode *MakeColumnRefNode(const std::string &column_name, const std::string &relation_name);
    ColumnIdNode *MakeColumnIdNode(size_t column_id);
    GetFieldExpr *MakeGetFieldExpr(ExprNode *input, const std::string &column_name, size_t column_id);
    ...
    BetweenExpr *MakeBetweenExpr(ExprNode *expr, ExprNode *left, ExprNode *right, const bool is_not_between);
    BinaryExpr *MakeBinaryExprNode(ExprNode *left, ExprNode *right, FnOperator op);
    ...
    AllNode *MakeAllNode(const std::string &relation_name, const std::string &db_name);
    ...
    FnNode *MakeFnHeaderNode(const std::string &name, FnNodeList *plist, const TypeNode *return_type);

    FnParaNode *MakeFnParaNode(const std::string &name, const TypeNode *para_type);
    ...
    FnIfElseBlock *MakeFnIfElseBlock(FnIfBlock *if_block, const std::vector<FnNode *> &elif_blocks,
    ...
    SqlNode *MakeCmdNode(node::CmdType cmd_type, const std::string &index_name, const std::string &table_name);
    SqlNode *MakeCreateIndexNode(const std::string &index_name, const std::string &table_name, ColumnIndexNode *index);
    ...
    node::FnForInBlock *MakeForInBlock(FnForInNode *for_in_node, FnNodeList *block);

    PlanNode *MakeSelectPlanNode(PlanNode *node);

    PlanNode *MakeGroupPlanNode(PlanNode *node, const ExprListNode *by_list);

    PlanNode *MakeProjectPlanNode(PlanNode *node, const std::string &table, const PlanNodeList &project_list,
                                  const std::vector<std::pair<uint32_t, uint32_t>> &pos_mapping);

    PlanNode *MakeLimitPlanNode(PlanNode *node, int limit_cnt);

    CreatePlanNode *MakeCreateTablePlanNode(const std::string &table_name, int replica_num, int partition_num,
                                            const NodePointVector &column_list,
                                            const NodePointVector &partition_meta_list);

    CreateProcedurePlanNode *MakeCreateProcedurePlanNode(const std::string &sp_name,
    ...
    SqlNode *MakeCreateProcedureNode(const std::string &sp_name, SqlNodeList *input_parameter_list,
    ...
    PlanNode *MakeUnionPlanNode(PlanNode *left, PlanNode *right, const bool is_all);
    ...
    ExternalFnDefNode *MakeExternalFnDefNode(const std::string &function_name, void *function_ptr,
                                             const node::TypeNode *ret_type, bool ret_nullable,
                                             const std::vector<const node::TypeNode *> &arg_types,
                                             const std::vector<int> &arg_nullable, int variadic_pos,
                                             bool return_by_arg);

    ExternalFnDefNode *MakeUnresolvedFnDefNode(const std::string &function_name);

    UdfDefNode *MakeUdfDefNode(FnNodeFnDef *def);

    UdfByCodeGenDefNode *MakeUdfByCodeGenDefNode(const std::string &name,
                                                 const std::vector<const node::TypeNode *> &arg_types,
                                                 const std::vector<int> &arg_nullable, const node::TypeNode *ret_type,
                                                 bool ret_nullable);

    UdafDefNode *MakeUdafDefNode(const std::string &name, const std::vector<const TypeNode *> &arg_types,
                                 ExprNode *init, FnDefNode *update_func, FnDefNode *merge_func, FnDefNode *output_func);
    LambdaNode *MakeLambdaNode(const std::vector<ExprIdNode *> &args, ExprNode *body);

    SqlNode *MakePartitionMetaNode(RoleType role_type, const std::string &endpoint);
    ...
    SqlNode *MakeInputParameterNode(bool is_constant, const std::string &column_name, DataType data_type);
 ...
 private:
    ProjectNode *MakeProjectNode(const int32_t pos, const std::string &name, const bool is_aggregation,
